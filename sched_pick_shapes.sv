...
module sched_pick_shapes (
  ...
  always_comb begin
    ...
    if (sw_a_i) begin
      ...
      tail_a = (ntok_a_i > s1_mdim_sel) ? (ntok_a_i - s1_mdim_sel) : '0;
      s2a_off = s1_ts_sel + best_s2_ticks(tail_a);
    end

    if (sw_b_i) begin
      s2b_off = best_s2_ticks(ntok_b_i);
    end else begin
      tail_b = (ntok_b_i > s1_mdim_sel) ? (ntok_b_i - s1_mdim_sel) : '0;
      s2b_off = s1_ts_sel + best_s2_ticks(tail_b);
    end
    ...
    if (dn_a_i || dn_b_i) begin
      s3a_o = SHAPE_C;
      s3b_o = SHAPE_C;
    end else if (abs_delta_ge_1) begin
