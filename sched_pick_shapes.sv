...
module sched_pick_shapes (
  ...
  always_comb begin
    ...
    // ── 估算两侧 S2 完成时刻（用于 S3 形状决策）──────────────────────────
    if (sw_a_i) begin
      // S1 跳过：S2 直接从 t0 开始，所有 ntok 进 S2
      s2a = t0_i + best_s2_t(ntok_a_i);
    end else begin
      tail_a = (ntok_a_i > mdim(s1a_o)) ? (ntok_a_i - mdim(s1a_o)) : '0;
      s2a = t0_i + kTs1(s1a_o) + best_s2_t(tail_a);
    end

    if (sw_b_i) begin
      s2b = t0_i + best_s2_t(ntok_b_i);
    end else begin
      tail_b = (ntok_b_i > mdim(s1b_o)) ? (ntok_b_i - mdim(s1b_o)) : '0;
      s2b = t0_i + kTs1(s1b_o) + best_s2_t(tail_b);
    end
